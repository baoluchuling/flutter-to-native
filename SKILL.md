...
- 仅当用户在 Step 5 confirm 中**针对特定功能项**明确同意简化（必须可追溯到对话中的具体文字，通用确认如"确认开始"不构成简化许可），或存在用户确认的真实外部阻塞（如后端接口未就绪，不得由 AI 自行判定为"外部阻塞"）时，才允许不完整输出；必须在 `cross_platform_gap.md` 和 `finalize_report.md` 的"遗留风险"中显式写出原因和未对齐项。
- **自动模式（`auto_mode: true`）下不存在简化许可**——Step 5 被跳过意味着没有人工确认环节，任何需求降级都不被允许。
