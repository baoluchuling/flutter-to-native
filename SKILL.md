...
- 允许架构映射，不允许需求降级。信息部分缺失时，必须在 `llm_plan.json` 对应 task 的 `assumptions` 字段中显式记录推断内容和依据，并在 Step 5 人工确认时展示给用户。不得静默推断后直接执行，不得自行发明需求。
...
- **AI 禁止提议简化**。仅当**用户主动提出**跳过或简化某个特定功能项时（如用户说"XX 功能先不做"），才允许不完整输出。AI 不得以任何方式建议、暗示或引导用户简化需求（如"这个比较复杂，是否考虑先做核心部分？"——禁止）。用户的通用确认（如"确认开始"）不构成简化许可。
- 存在用户确认的真实外部阻塞（如后端接口未就绪，不得由 AI 自行判定为"外部阻塞"）时，也允许不完整输出。
- 允许不完整输出时，必须在 `cross_platform_gap.md` 和 `finalize_report.md` 的"遗留风险"中显式写出原因和未对齐项。
...
以下是 AI 在执行 SOP 时最常见的偷懒借口及其封堵。如果你发现自己正在产生表中的任何理由，**立即停止并执行正确做法**。

*映射偷懒*

| 借口 | 现实 |
|---|---|
| "Flutter 没有直接对应的 Native 控件，跳过" | 没有直接对应不等于不需要实现。用 Native 方式等价实现行为，而不是跳过。 |
| "Flutter 的回调链太深了，Native 不需要这么复杂" | 回调链每一层都有原因（错误传播、状态同步、生命周期）。简化链路 ≠ 删除链路。 |
| "Native 有更好的方式，不需要按 Flutter 结构来" | 架构可以映射，行为不能改变。实现模式可以不同，用户可见行为必须一致。 |
| "这个 lifecycle 回调 Native 会自动处理" | Flutter 显式处理的 lifecycle 逻辑不会在 Native 自动发生。必须在 viewWillAppear / onResume 等节点显式实现。 |

*集成遗漏*

| 借口 | 现实 |
|---|---|
| "集成点我大概知道在哪，不需要精确到方法级" | 必须精确到 file:method:line。"大概"= 没找到 = 没验证 = 可能是死代码。 |
| "这个异步时序在 Native 上不一样，先不处理" | 时序不同恰恰说明需要显式处理。Flutter 做了竞态保护的地方，Native 更容易出竞态。 |

*流程跳步*

| 借口 | 现实 |
|---|---|
| "Step 4 校验太慢了，代码我很有信心" | 校验花 5 分钟，修线上 bug 花 5 天。自信不是质量保证，Gate Checklist 是。 |
| "Step 7 code review 和 Step 6 检查重复了" | 粒度不同（task 级 vs 全局），审查者不同（Claude vs 独立 AI）。不同 AI 审同一份代码能发现彼此的系统性盲区，这不是重复，是独立验证。 |
| "这个 Gate Checklist 项不适用于当前场景" | 不适用需要用证据证明（如 grep 确认无 UI 变更），不是凭感觉跳过。 |
| "编译通过了就行，不需要 verify" | 编译通过只说明语法正确。行为对齐、链路完整、资产齐全都需要 verify 检查。 |
| "understand-explain 太慢了，我直接看代码" | 你看到的是局部，understand-explain 看到的是调用链全貌。跳过它就可能映射到错误的编排入口。 |

*双端同步*

| 借口 | 现实 |
|---|---|
| "iOS 先做完，Android 参考 iOS 来就行" | 两端架构不同，照搬会出错。必须各自走完 Step 2.4+ 的独立分析和映射。 |
| "两端差异不大，不需要写 cross_platform_gap.md" | 差异不管大小都要留档。不留档 = 没人知道差异存在 = 下次踩同一个坑。 |
| "一端 FAIL 了，另一端也暂停等它" | SOP 明确规定各端独立推进。一端 FAIL 不阻塞另一端继续。 |
| "双端 task 结构应该一样，复制 edit_tasks.json 改改就行" | 两端的 native_chain_match 结果不同，task 的 target_file、integration_point、行为映射都可能不同。必须独立生成。 |

*安全相关*

| 借口 | 现实 |
|---|---|
| "Flutter 那边用 SharedPreferences 存 Token，Native 也这样就行" | Flutter 的 flutter_secure_storage 底层就是 Keychain/Keystore。Native 必须用同等安全级别的存储，不能降级到明文。 |
| "安全是后端的事，客户端不用管" | 客户端存储、日志、证书验证、组件导出都是攻击面。参考 security-hardening skill。 |
| "这个深链接参数不需要验证" | 深链接是外部入口，任何人可以构造任意参数。必须在 Native 侧验证参数合法性。 |
...
各步骤引用文档中使用 `flutter/xxx` 和 `<platform>/xxx` 作为路径前缀。实际路径根据同步模式解析：

| 引用写法 | 双端同步 | 单端同步 |
|---------|---------|---------|
| `flutter/hunk_facts.json` | `<run-dir>/flutter/hunk_facts.json` | `<run-dir>/hunk_facts.json` |
| `<platform>/edit_tasks.json` | `<run-dir>/ios/edit_tasks.json` 或 `<run-dir>/android/edit_tasks.json` | `<run-dir>/edit_tasks.json` |
| `finalize_report.md` | `<run-dir>/finalize_report.md`（始终在根目录） | 同左 |

**规则**：读取 `session_config.json` 的 `platforms` 字段——若为单平台，所有产物平铺在 `<run-dir>/` 下，无子目录前缀；若为双平台，共享产物在 `flutter/`，平台产物在 `ios/` 或 `android/`。
...
| 5 | confirm | 人工审阅 edit_tasks + plan_validation（`auto_mode: true` 时自动跳过） | 按平台 | [05-confirm.md](references/05-confirm.md) |
| 6 | execute | 准备(6A) → 执行+检查(6B, Claude) → 收尾(6C) → 编译验证(6D) | 按平台 | [06-execute.md](references/06-execute.md) |
| 7 | code_review | 独立 AI 全局审查（Codex 优先），强制执行 | 按平台 | [07-code-review.md](references/07-code-review.md) |
