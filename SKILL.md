...
在 run 目录维护 `token_usage.md`，记录整个 SOP 流程的 token 消耗。

**数据来源：**

Claude Code 的 session JSONL 文件包含每次 API 调用的完整 usage 信息。路径：
```
~/.claude/projects/<project-hash>/<session-id>.jsonl
```
每条带 `"usage"` 字段的记录包含：`input_tokens`、`output_tokens`、`cache_creation_input_tokens`、`cache_read_input_tokens`。

**操作流程：**

1. **SOP 开始时（Step 0 之前）**：记录当前 session JSONL 的行数作为起点
   ```bash
   SESSION_JSONL=$(ls -t ~/.claude/projects/*/$(ls -t ~/.claude/projects/*/*.jsonl | head -1 | xargs basename) 2>/dev/null | head -1)
   START_LINE=$(wc -l < "$SESSION_JSONL")
   ```
   将 `SESSION_JSONL` 路径和 `START_LINE` 写入 `<run-dir>/token_tracking.json`

2. **每次 Agent 调用返回后**：从返回结果中提取 `total_tokens`（在 `<usage>` 标签中），追加到 `<run-dir>/token_usage.md`

3. **SOP 结束时（Step 9 finalize）**：从 session JSONL 提取 START_LINE 之后的所有 usage 记录，汇总主 session 的精确 token 用量
   ```python
   import json, re
...
|---|------|------|-------|--------|------|
| 1 | Step 6 | TASK-01 ChargeModel | haiku | 12,345 | 15s |
| ... | | | | | |
...
| 项目 | Tokens | 费用 |
|------|--------|------|
| Input (新) | xxx | $x.xx (@$15/MTok) |
| Input (缓存命中) | xxx | $x.xx (@$1.875/MTok) |
| Output | xxx | $x.xx (@$75/MTok) |
...
> 每次 Agent 返回后必须追加 subagent 记录，不得事后补填。finalize_report 必须包含完整 token_usage 汇总。
...
2. 执行 `git log --oneline` 确认 Step 6 开始前的 commit SHA（即 `execution_log.md` 中第一条记录之前的 commit）
3. 执行 `git reset --hard <pre-execution-commit>` 回到 Step 6 前的状态
4. 保留 run 目录中的所有产物（不删除，供后续分析）
5. 根据中止原因决定恢复路径：
