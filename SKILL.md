...
**Git 提交策略**
- **Step 0-5（规划阶段）**：不提交 git。产物仅写入 run 目录，不污染仓库历史。
- **Step 6（执行阶段）**：所有 task 代码改动完成 + Gate Checklist 全部 PASS 后，**统一一次 commit**。禁止每个 task 单独 commit。
- **Step 7-8（审查/验收）**：若有修复代码，修复完成后单独 commit。
- **subagent 执行时**：在 subagent prompt 中明确指示 **不要 commit**，仅编写代码。由主 session 在 Gate Checklist 通过后统一 commit。
