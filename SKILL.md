...
**记录时机：**
- 每次 Agent 工具调用返回后，从返回结果中提取 `total_tokens`（在 `<usage>` 标签中），立即追加到 `token_usage.md`
- 主 session 无法精确统计，在 finalize 时按对话轮次估算
...
| 2 | Step 6 | TASK-08 V2 弹窗 | opus | 56,789 | 380s |
| ... | | | | | |

## 汇总

| 模型 | 调用次数 | 总 Tokens | 预估费用 |
|------|---------|----------|---------|
| haiku | 6 | xxx | $x.xx |
| sonnet | 4 | xxx | $x.xx |
| opus | 3 | xxx | $x.xx |
...
主 session 预估：~xxx tokens（按 xx 轮对话估算）
```

**费用计算公式**（按 2025 年定价，input/output 混合估算）：
- haiku: $0.001/K tokens（input $0.25 + output $1.25 混合）
- sonnet: $0.008/K tokens（input $3 + output $15 混合）
- opus: $0.025/K tokens（input $15 + output $75 混合）

> 每次 Agent 返回后必须追加记录，不得事后补填。finalize_report 必须包含 token_usage 汇总。
