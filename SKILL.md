...
description: "Flutter 需求到 Native（iOS/Android）的流程编排型经验工具（SOP）：结构建模→Flutter变更采集→意图提炼→能力任务规划→校验闸门→人工确认→CLI执行→验收闭环→交付。支持单端或双端同时同步。"
---
...
| 3 | plan | 能力任务规划，执行 `atlas_planner.py plan` | 按平台 | [03-plan.md](references/03-plan.md) |
| 4 | plan_validation | 自动校验闸门（V7-V14），FAIL 禁止执行 | 按平台 | [04-plan-validation.md](references/04-plan-validation.md) |
...
| 6.5 | code_review | 全局代码审查，强制执行 | 按平台 | [06.5-code-review.md](references/06.5-code-review.md) |
| 7 | verify | 验收闭环 + 基准测试，执行 `atlas_verify.py verify` | 按平台 | [07-verify.md](references/07-verify.md) |
| 8 | finalize | 交付汇总 + 最终自检 | 合并 | [08-finalize.md](references/08-finalize.md) |
...
| `Step 1 — Flutter 变更采集` | Flutter 变更采集 + Figma 截图 | `采集 Flutter 变更` |
| `Step 2 — 意图提炼与映射` | 需求意图提炼 + 自动映射子流程 + llm_plan 生成 | `提炼需求意图` |
| `Step 3 — 能力任务规划` | 能力任务规划，生成 edit_tasks | `生成任务规划` |
| `Step 4 — 校验闸门` | 自动校验闸门（V7-V14） | `执行校验闸门` |
| `Step 5 — 人工确认` | 人工审阅 edit_tasks + plan_validation | `等待人工确认` |
| `Step 6 — 执行` | CLI 改码 + 编译验证 | `执行代码同步` |
| `Step 6.5 — 代码审查` | 全局代码审查 | `执行代码审查` |
| `Step 7 — 验收` | 验收闭环 + 基准测试 | `执行验收检查` |
| `Step 8 — 交付` | 交付汇总 + 最终自检 | `生成交付汇总` |
...
- Step 6.5 blockedBy Step 6
- Step 7 blockedBy Step 6.5
