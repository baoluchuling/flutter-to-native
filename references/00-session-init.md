...
```json
{
  "questions": [{
    ...
    "header": "确认模式",
    "options": [
      {"label": "标准模式", "description": "Step 5 人工审阅 edit_tasks，流程中需要用户确认时暂停等待"},
      {"label": "自动模式", "description": "跳过 Step 5 人工确认，plan_validation PASS 后直接进入执行"}
    ],
    "multiSelect": false
  }]
}
...
- **标准模式**（`auto_mode: false`）：Step 5 正常执行，用户审阅后确认
- **自动模式**（`auto_mode: true`）：Step 5 自动跳过（plan_validation PASS/WARN 即进入 Step 6），不允许简化许可（因为没有人工确认环节，任何需求降级都不被允许）

将选择结果写入 `session_config.json` 的 `auto_mode` 字段。
