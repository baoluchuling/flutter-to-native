...
用户确认后、创建 run 目录时，同步初始化 token 追踪基线：

1. 定位当前 session 的 JSONL 文件（Claude Code 存储路径）：
   ```bash
   # 找当前工作目录对应的 project 目录下最新的 jsonl
   PROJECT_DIR=$(ls -dt ~/.claude/projects/*/ | head -1)
   SESSION_JSONL=$(ls -t "${PROJECT_DIR}"*.jsonl | head -1)
   START_LINE=$(wc -l < "$SESSION_JSONL")
   ```

2. 将基线信息写入 `<run-dir>/token_tracking.json`：
   ```json
   {
...
3. 创建空的 `<run-dir>/token_usage.md`，写入表头：
   ```markdown
   # Token Usage
