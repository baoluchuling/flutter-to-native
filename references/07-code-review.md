...
- 用户在 Step 5 (confirm) 中**针对特定功能项**明确同意简化（必须可追溯到 Step 5 对话中的具体文字，如"XX 功能可以先不做"）。Step 5 中的通用确认（如"确认开始"）不构成简化许可。**自动模式（`auto_mode: true`）下此条件永远不成立**——Step 5 被跳过，无人工确认环节，不允许任何简化
