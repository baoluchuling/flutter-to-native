...
> **双端同步**：Step 2.1 ~ 2.3 是纯 Flutter 分析，只执行一次，产物存入 `flutter/` 共享目录。Step 2.4 ~ 2.5 需要查询 Native 代码，按平台各执行一轮，产物存入 `<platform>/` 目录。
...
产物：`flutter/flutter_chain_map.json`（含可选 `uncovered_facts` 字段）

### 2.4 native_chain_match（按平台）

> 双端同步时，每个平台各执行一次，产物分别存入 `ios/` 和 `android/`。
...
产物：`<platform>/mapping_disambiguation.md`
...
- `flutter/flutter_changes.md`（共享）
- `pr_diff` 原文（共享）
- `figma_inputs`（链接与截图，共享）
- Native 代码理解结果（按平台）

最少字段：
- `meta`（含 `analysis_mode / generated_by / platform / evidence.pr_diff_path / evidence.pr_diff_sha256`）
- `meta.mapping_pipeline`（含 `capability_split / flutter_hunk_extract / flutter_chain_extract / native_chain_match / disambiguation`）
- `intent_markdown`
- `hunk_facts`（json，来自 `flutter_hunk_extract`，共享引用）
- `capability_slices`（markdown 文本，共享引用）
- `flutter_chain_map`（json 对象，共享引用）
- `native_chain_candidates`（json 对象，平台独立）
